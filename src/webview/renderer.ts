import { LAYERS, ELEMENT_TYPES, RELATIONSHIP_TYPES, ArchimateModel, ArchimateElement, ArchimateRelationship, ElementTypeDef, Point } from './model.js';
...
function handlePosition(handle: ResizeHandle, w: number, h: number): [number, number] {
  const x = handle.includes('w') ? 0 : handle.includes('e') ? w : w / 2;
  const y = handle.includes('n') ? 0 : handle.includes('s') ? h : h / 2;
  return [x, y];
}

export function shapeElement(g: SVGGElement, el: { w: number; h: number }, def: ElementTypeDef): void {
  const { w, h } = el;
  const stroke = LAYERS[def.layer].stroke;
  const fill = LAYERS[def.layer].color;
  const common: SvgAttrs = { fill, stroke, 'stroke-width': 1.5, class: 'am-shape' };
  switch (def.shape) {
    case 'oval': {
      g.appendChild(svgEl('ellipse', { cx: w / 2, cy: h / 2, rx: w / 2, ry: h / 2, ...common }));
      break;
    }
    case 'rounded': {
      g.appendChild(svgEl('rect', { x: 0, y: 0, width: w, height: h, rx: 10, ry: 10, ...common }));
      break;
    }
    case 'component': {
      g.appendChild(svgEl('rect', { x: 0, y: 0, width: w, height: h, ...common }));
      for (const ty of [h * 0.22, h * 0.62]) {
        g.appendChild(svgEl('rect', { x: -8, y: ty, width: 16, height: 10, fill, stroke, 'stroke-width': 1.3 }));
      }
      break;
    }
    case 'box3d': {
      ...
      g.appendChild(svgEl('path', {
        d: `M0,${d} L${d},0 L${w},0 L${w},${h - d} L${w - d},${h} L0,${h} Z`, ...common,
      }));
      g.appendChild(svgEl('path', { d: `M0,${d} L${w - d},${d} L${w},0`, fill: 'none', stroke, 'stroke-width': 1 }));
      g.appendChild(svgEl('path', { d: `M${w - d},${d} L${w - d},${h} M${w - d},${d} L${w},0`, fill: 'none', stroke, 'stroke-width': 1 }));
      break;
    }
    case 'chevron': {
      const nick = 14;
      g.appendChild(svgEl('path', { d: `M0,0 L${w - nick},0 L${w},${h / 2} L${w - nick},${h} L0,${h} L${nick},${h / 2} Z`, ...common }));
      break;
    }
    case 'wavyrect': {
      g.appendChild(svgEl('rect', { x: 0, y: 0, width: w, height: h, ...common }));
    ...
    case 'junction': {
      g.appendChild(svgEl('circle', { cx: w / 2, cy: h / 2, r: Math.min(w, h) / 2, fill: stroke, stroke }));
      break;
    }
    case 'rect':
    default: {
...
export function badgeGlyph(def: ElementTypeDef): SVGGElement {
  ...
  switch (def.badge) {
    case 'actor':
      ...
      s.appendChild(svgEl('path', { d: 'M7,5.6 L7,11 M3.5,8 L10.5,8 M7,11 L4,15 M7,11 L10,15', ...common }));
      break;
    case 'process': case 'function': case 'interaction':
      s.appendChild(svgEl('path', { d: 'M1,7 L11,7 M8,3.5 L12,7 L8,10.5', ...common }));
      break;
    case 'service':
      s.appendChild(svgEl('rect', { x: 1, y: 3, width: 12, height: 8, rx: 4, ...common }));
      break;
    case 'interface':
      s.appendChild(svgEl('circle', { cx: 4, cy: 7, r: 3, ...common }));
      s.appendChild(svgEl('path', { d: 'M7,7 L13,7', ...common }));
      break;
    case 'object': case 'artifact':
      s.appendChild(svgEl('rect', { x: 1, y: 1, width: 12, height: 12, ...common }));
      s.appendChild(svgEl('path', { d: 'M1,5 L13,5', ...common }));
      break;
    case 'event':
      s.appendChild(svgEl('path', { d: 'M1,3 L8,3 L13,7 L8,11 L1,11 Z', ...common }));
      break;
    case 'goal':
      s.appendChild(svgEl('circle', { cx: 7, cy: 7, r: 6, ...common }));
      s.appendChild(svgEl('circle', { cx: 7, cy: 7, r: 3, ...common }));
      break;
    case 'requirement': case 'principle': case 'constraint':
      s.appendChild(svgEl('path', { d: 'M2,12 L2,3 L12,3 L12,12', ...common }));
      s.appendChild(svgEl('path', { d: 'M2,12 L12,12', stroke, 'stroke-width': 2.2 }));
      break;
    case 'value': case 'outcome': case 'meaning':
      s.appendChild(svgEl('path', { d: 'M7,1 L12,13 L1,13 Z', ...common }));
      break;
    case 'capability': case 'resource': case 'course':
      s.appendChild(svgEl('circle', { cx: 7, cy: 7, r: 6, ...common }));
      s.appendChild(svgEl('path', { d: 'M4,7 L10,7 M7,4 L7,10', ...common }));
      break;
    case 'node': case 'device': case 'system': case 'equipment': case 'facility':
      s.appendChild(svgEl('path', { d: 'M1,4 L5,1 L13,1 L13,9 L9,12 L1,12 Z', ...common }));
      break;
    default:
      s.appendChild(svgEl('circle', { cx: 7, cy: 7, r: 6, ...common }));
  }
...
export type HingePointerHandler = (e: PointerEvent, relId: string, end: 'source' | 'target') => void;
export type ResizeHandlePointerHandler = (e: PointerEvent, elId: string, handle: ResizeHandle) => void;
...
export class Renderer {
  ...
  constructor(svg: SVGSVGElement, model: ArchimateModel, callbacks: RendererCallbacks = {}) {
    this.svg = svg;
    this.model = model;
  ...
  clearGuides(): void { this.guideLayer.replaceChildren(); }
  ...
  renderElement(el: ArchimateElement): void {
    ...
    const g = svgEl('g', { class: `am-element am-layer-${def.layer}`, transform: `translate(${el.x},${el.y})`, 'data-id': el.id });
    this._paintElement(g, el, def);
    if (this.onElementPointerDown) g.addEventListener('pointerdown', (e) => this.onElementPointerDown!(e, el.id));
  ...
  private _paintElement(g: SVGGElement, el: ArchimateElement, def: ElementTypeDef = ELEMENT_TYPES[el.type]): void {
    g.replaceChildren();
    shapeElement(g, el, def);
    if (def.badge) {
      const badge = badgeGlyph(def);
      badge.setAttribute('transform', `translate(${el.w - 18},4)`);
      g.appendChild(badge);
    }
    const fo = svgEl('foreignObject', { x: 4, y: 2, width: Math.max(el.w - 8, 1), height: Math.max(el.h - 4, 1) });
    const div = document.createElement('div');
    div.className = 'am-label';
    div.textContent = el.name;
    fo.appendChild(div);
    g.appendChild(fo);
    const typeLabel = svgEl('text', { x: 4, y: el.h - 4, class: 'am-type-label' });
    ...
    for (const handle of RESIZE_HANDLES) {
      ...
      const rect = svgEl('rect', {
        class: `am-resize-handle am-resize-${handle}`, 'data-handle': handle,
        x: hx - size / 2, y: hy - size / 2, width: size, height: size,
      });
      if (this.onResizeHandlePointerDown) {
        rect.addEventListener('pointerdown', (e) => { e.stopPropagation(); this.onResizeHandlePointerDown!(e, el.id, handle); });
      }
      g.appendChild(rect);
    }
  }
  ...
  obstaclesExcept(...ids: string[]) {
    ...
    return [...this.model.elements.values()].filter(e => !skip.has(e.id)).map(e => e.bounds());
  }
  ...
  renderEdge(rel: ArchimateRelationship): Point[] | undefined {
    ...
    if (!g) {
      ...
      const hit = svgEl('path', { class: 'am-edge-hit', fill: 'none', stroke: 'transparent', 'stroke-width': 14 });
      ...
      const sourceHandle = svgEl('circle', { class: 'am-edge-handle', r: 5, 'data-end': 'source' });
      const targetHandle = svgEl('circle', { class: 'am-edge-handle', r: 5, 'data-end': 'target' });
      if (this.onHingePointerDown) {
        sourceHandle.addEventListener('pointerdown', (e) => { e.stopPropagation(); this.onHingePointerDown!(e, rel.id, 'source'); });
        targetHandle.addEventListener('pointerdown', (e) => { e.stopPropagation(); this.onHingePointerDown!(e, rel.id, 'target'); });
      }
      g.appendChild(sourceHandle);
      g.appendChild(targetHandle);
      if (this.onEdgeClick) g.addEventListener('pointerdown', (e) => { e.stopPropagation(); this.onEdgeClick!(e, rel.id); });
      this.edgeLayer.appendChild(g);
      this.edgeDom.set(rel.id, g);
    }
    const pts = rel.bendpoints && rel.bendpoints.length
      ? rel.bendpoints
      : this.router.routeEdge(rel.id, src.bounds(), tgt.bounds(), this.obstaclesExcept(src.id, tgt.id), { sourcePort: rel.sourcePort, targetPort: rel.targetPort });
    ...
    line.setAttribute('stroke-dasharray', def.style === 'dashed' ? '7,4' : def.style === 'dotted' ? '1.5,3.5' : 'none');
    if (def.endMarker) line.setAttribute('marker-end', `url(#am-${def.endMarker})`); else line.removeAttribute('marker-end');
    if (def.startMarker) line.setAttribute('marker-start', `url(#am-${def.startMarker})`); else line.removeAttribute('marker-start');
  ...
  rerouteConnected(elementId: string): void {
    for (const rel of this.model.relationships.values()) {
      if (rel.source === elementId || rel.target === elementId) {
        if (!rel.bendpoints) this.renderEdge(rel);
      }
    }
  }

  rerouteAll({ preserveManual = true }: { preserveManual?: boolean } = {}): void {
    this.router.resetLanes();
    for (const rel of this.model.relationships.values()) {
  ...
  setSelected(ids: Set<string>): void {
    for (const [id, g] of this.elementDom) g.classList.toggle('am-selected', ids.has(id));
    for (const [id, g] of this.edgeDom) g.classList.toggle('am-selected', ids.has(id));
  }
}
